...
function showBestAndWorstGames(pointsGames) {
  var timeBeated = 0;
  var worstBeated = 0;
  let lowerPoint = Infinity;
  let mostPoint = -Infinity;
  let games = pointsGames.split(" ");
  for (let i = 0; i < games.length; i++) {
    const value = parseInt(games[i]);
    if (value > mostPoint && i > 0) {
      timeBeated++;
      mostPoint = value;
    }
  ...
  return [timeBeated, worstBeated];
}

console.log(showBestAndWorstGames("10 20 20 8 25 3 0 30 1"));
